...
class PointLineView( QWidget ):
	...
	def paintEvent(self, event):						  
		...
		for color in self.lineList:
			...
			painter.setPen( c )
			# print("Blah : ", self.lineList[color])
			if(type(self.lineList[color]) == QLineF):
				ln = QLineF(w*self.lineList[color].x1(), h*self.lineList[color].y1(), w*self.lineList[color].x2(), h*self.lineList[color].y2())
				painter.drawLine(ln)
			else:
...
# Methods that handle GUI events
	def clearClicked(self):
		...
		self.solveButtonDivideandConquer.setEnabled(True)
		self.view.update()								
		app.processEvents()							
	...
	def solveClicked(self):
		...
		# self.solveButtonDivideandConquer.setEnabled(False)
		self.view.update()
		app.processEvents()							
		self.solver.compute_hull(self.points,self.showRecursion.isChecked(),self.view)
		# self.solver.compute_hull_divide_and_conquer(self.points,self.showRecursion.isChecked(),self.view)
		self.generateButton.setEnabled(True)
		self.clearButton.setEnabled(True)
		self.view.update()
		app.processEvents()

	def solveClickedDandC(self):
		self.generateButton.setEnabled(False)
		self.clearButton.setEnabled(False)
		# self.solveButton.setEnabled(False)
		self.solveButtonDivideandConquer.setEnabled(False)
...
# Setting up the GUI
	def initUI( self ):
		...
		self.solveButtonDivideandConquer    = QPushButton('SolveDandC')
		...
		h.addWidget(self.solveButtonDivideandConquer)
		...
		# self.solveButtonDivideandConquer.clicked.connect(self.solveClicked)
		self.clearButton.clicked.connect(self.clearClicked)

		self.generateButton.clicked.connect(self.generateClicked)
		# self.solveButton.clicked.connect(self.solveClicked)
		self.solveButtonDivideandConquer.clicked.connect(self.solveClickedDandC)
		self.clearButton.clicked.connect(self.clearClicked)
